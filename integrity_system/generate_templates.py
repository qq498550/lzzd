...
def main():
    ...
    discipline_headers = ['姓名*', '分公司', '部门', '职务', '处理机构',
                          '问责情况*', '问责时间*', '有无影响期', '影响期截止日期', '事由*', '备注']
    discipline_sample = [
        ['张三', '第一分公司', '财务部', '经理', '纪委', '警告', '2024-01-15', '有', '2025-01-14', '违反财务规定', ''],
        ['李四', '第二分公司', '销售部', '', '组织部', '记过', '2024-03-20', '有', '2026-03-19', '业绩造假', ''],
    ]
    discipline_instructions = "【违纪记录导入模板】*号为必填项。有无影响期填：有/无。日期格式：YYYY-MM-DD"
    create_template(
    ...
    violation_headers = ['姓名*', '分公司', '部门', '职务', '处理机构',
                         '问责类型*', '问责时间*', '有无影响期', '影响期截止日期', '事由*', '备注']
    violation_sample = [
        ['王五', '第三分公司', '采购部', '主管', '监察部', '严重警告', '2023-06-10', '有', '2025-06-09', '违规采购', ''],
        ['赵六', '第一分公司', '人事部', '', '组织部', '诫勉谈话', '2024-02-28', '无', '', '考勤违规', ''],
    ]
    violation_instructions = "【违规记录导入模板】*号为必填项。有无影响期填：有/无。日期格式：YYYY-MM-DD"
    create_template(
    ...
    petition_headers = ['姓名*', '分公司', '部门', '举报日期*', '举报内容*',
                         '核查结果', '组织是否采信', '有无影响期', '影响期截止日期', '备注']
    petition_sample = [
        ['孙七', '第二分公司', '市场部', '2024-05-01', '反映贪污受贿问题', '查无实据', '是', '无', '', ''],
        ['周八', '第一分公司', '技术部', '2024-06-15', '反映违规操作', '', '', '无', '', ''],
    ]
    petition_instructions = "【信访举报导入模板】*号为必填项。组织是否采信填：是/否/空。有无影响期填：有/无/空。日期格式：YYYY-MM-DD"
    create_template(
